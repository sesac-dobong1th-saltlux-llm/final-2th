...
const Banner = () => {
    ...
    const findResult = () => {
        if (find.length === 0 && word.length > 0) {
            return <div className="find-search">일치항목 없음</div>
        }
        if (find.length > 0) {
                    ...
                    <div className="row">
                        <div className="col-lg-6 mx-auto">
                            <div className="banner-area text-center pt-4 pb-4">
                                <p>당신에게 딱 맞는 집을 추천해드립니다!</p>
                                <h2 className="mt-2 mb-4 banner-title"><strong> 나의 집 찾기</strong> </h2>
                                <div className="search-area">
                                    <input value={word} onChange={(e) => findSearch(e)} type="text" className="inp-search" placeholder="검색" />
                                    <button className="btn-search m-2">찾기</button>
                                </div>
                                {findResult()}
                            </div>
                        </div>
